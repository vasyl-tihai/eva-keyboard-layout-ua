...
спліт-клавіатурі її проблеми видно в цифрах (корпус ~250 000 літер, [analyzer.py](analyzer.py)):

- **Перевантаження вказівних пальців.** Лівий вказівний тримає К, Е, А, П, М, И — 28.6 %
  усіх натискань; правий — Н, Г, Р, О, Т, Ь — ще 30.9 %. **Разом два пальці роблять 59.5 % роботи.**
- **Високий SFB** (Same Finger Bigram — дві літери поспіль одним пальцем). У ЙЦУКЕН
  таких **18 % усіх біграм**: «ро» (1.5 %), «но» (1.2 %), «ор», «го», «то», «ка», «ки»…
...
Пораховано скриптом [analyzer.py](analyzer.py) на корпусі з ~250 000 літер (випадкові статті
української Вікіпедії; замініть на власні тексти — цифри трохи зміняться).
Модель чесна: внутрішня колонка належить вказівному пальцю, біграми рахуються
лише всередині слів (пробіл на великому пальці розриває їх).

| Розкладка | SFB % ↓ | Чергування рук % ↑ | Домашній ряд % ↑ | Мізинці % ↓ |
|---|---|---|---|---|
| ЙЦУКЕН | 18.08 | 54.8 | 46.5 | 8.7 |
| **Ева 3.8** | **3.29** | **57.8** | **57.8** | 17.4 |
| Ліна | 1.01 | 76.2 | 57.4 | 24.8 |
| Canary Phonetic UA | 4.65 | 59.5 | 49.3 | 13.9 |

**Ева 3.8 має у 5.5 раза менше SFB, ніж ЙЦУКЕН**, і тримає 58 % набору на домашньому ряду.
...
### 🏆 Найкращий вибір: Ева 3.8

Найновіша версія. У 3.8 з основної зони прибрано клавішу Delete, що дозволило
поставити кому та крапку на зручні позиції нижнього ряду. Файл розкладки: [Eva.vil](Eva.vil)

<p align="center">
  <img src="images/eva-3.8.png" alt="Розкладка Ева 3.8 — фінальна версія з комою та крапкою в нижньому ряду" width="600">
</p>
...
2. Відкрийте програму Vial → **File → Load saved layout** → виберіть `Eva.vil`.
...
- **Ева 3.8 — фінальна пунктуація** (див. вище).
...
SFB у Ліни — 1.01 %, чергування рук — 76 %. Слабке місце — 24.8 % навантаження на
...
перемикання мови не ламає звичок. Чесна ціна: SFB 4.65 % — вищий, ніж у Еви
...
[ПРОПОЗИЦІЇ.md](ПРОПОЗИЦІЇ.md): «Ева 3.8.1» (два обміни, −36 % SFB),
«Ева 3.9» (три обміни, −42 % SFB), «Ева 3.9+» та «Ліна 2.0».
...
- Частотні дані: власний корпус із випадкових статей [української Вікіпедії](https://uk.wikipedia.org/) (~250 000 літер), скрипт [analyzer.py](analyzer.py).
